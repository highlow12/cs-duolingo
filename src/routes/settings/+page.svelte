<script lang="ts">
  ...
  function exportData() { return action(async()=>{const json=await learningRepository.exportBackup();const url=URL.createObjectURL(new Blob([json],{type:'application/json'}));const anchor=document.createElement('a');anchor.href=url;anchor.download=`cs-duolingo-backup-${new Date().toISOString().slice(0,10)}.json`;anchor.click();setTimeout(()=>URL.revokeObjectURL(url),30000);},'백업 파일을 만들었습니다. 다운로드한 파일을 보관해 주세요.'); }
  async function selectFile(event:Event) { const input=event.target as HTMLInputElement;const file=input.files?.[0];input.value='';if(!file)return;error='';message='';if(file.size>20*1024*1024){error='20MB 이하의 백업 파일을 선택해 주세요.';return;}try{importData=await file.text();importName=file.name;}catch(e){error=errorMessage(e);} }
  function restore() { if(!importData)return;const json=importData;return action(async()=>{await learningRepository.importBackup(json);importData=null;await load();},'백업을 복원했습니다. 학습 기록을 확인해 주세요.'); }
