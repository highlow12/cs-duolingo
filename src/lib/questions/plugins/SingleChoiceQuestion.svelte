<script lang="ts">
  ...
  type Props = QuestionRendererProps<"single-choice">;
  let {
    question,
    disabled,
    attemptKey,
  ...
  function selectOption(optionId: string) {
    if (disabled) return;
    selectedId = optionId;
    onAnswerChange({ type: "single-choice", optionId });
  }

  function isSubmitted(optionId: string): boolean {
    return submittedAnswer?.type === "single-choice" && submittedAnswer.optionId === optionId;
  }

  function isCanonical(optionId: string): boolean {
    return reveal?.type === "single-choice" && reveal.optionId === optionId;
  }
</script>
  ...
  <div class="options" role="radiogroup" aria-label="답 선택">
    {#each displayedOptions as option}
      ...
      <button
        ...
        aria-label={`${contentLabel(option.content)}${submitted ? ", 내 답" : ""}${canonical ? ", 정답" : ""}`}
        disabled={disabled}
