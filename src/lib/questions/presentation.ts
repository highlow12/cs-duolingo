...
function optionLabel(question: SingleChoiceQuestion | MultiSelectQuestion, id: string): string {
...
function matchingLabel(question: MatchingQuestion, side: "left" | "right", id: string): string {
...
export function describeAnswer(question: Question, answer: UserAnswer): AnswerDisplay {
...
export function describeCanonicalAnswer(question: Question, answer: CanonicalAnswer): AnswerDisplay {
  switch (question.type) {
    case "single-choice": {
      const candidate = answer as Extract<CanonicalAnswer, { type: "single-choice" }>;
      return {
        kind: "single",
        title: "정답",
        values: [optionLabel(question, candidate.optionId)],
    ...
    case "multi-select": {
      const candidate = answer as Extract<CanonicalAnswer, { type: "multi-select" }>;
      return {
        kind: "list",
        title: "정답",
        values: candidate.optionIds.map((id) => optionLabel(question, id)),
    ...
    case "fill-blank": {
      const candidate = answer as Extract<CanonicalAnswer, { type: "fill-blank" }>;
      return { kind: "value", title: "정답", value: candidate.value };
    }
    case "ordering": {
      const candidate = answer as Extract<CanonicalAnswer, { type: "ordering" }>;
      return {
        kind: "list",
        title: "정답 순서",
        values: candidate.orderedItemIds.map((id) => orderingLabel(question, id)),
    ...
    case "matching": {
      const candidate = answer as Extract<CanonicalAnswer, { type: "matching" }>;
      return {
        kind: "pairs",
        title: "정답 짝",
        pairs: candidate.pairs.map((pair) => ({
    ...
    case "code-output": {
      const candidate = answer as Extract<CanonicalAnswer, { type: "code-output" }>;
      return {
        kind: "value",
        title: "정답 출력",
        value: valueLabel(candidate.value),
    ...
    case "code-completion": {
      const candidate = answer as Extract<CanonicalAnswer, { type: "code-completion" }>;
      return {
        kind: "blanks",
        title: "정답 빈칸",
        blanks: question.blanks.map((blank, index) => ({
